...
def visit(visit, id):
    ...
    # Добавляем детали к событиям
    for event in current_visit['events']:
        # Получаем детали события из EVENTS и добавляем их к событию
        event_details = next((e for e in EVENTS if e['id'] == event['id']), {})
        event['duration'] = event_details.get('duration')
        event['description'] = event_details.get('description')
        event['type'] = event_details.get('type')
        event['img_url'] = event_details.get('img_url')

    return render(visit, 'visit.html', {'current_visit': current_visit})

    ...
    def get(self, request):
        if request.user.is_staff:
            if 'date' in request.GET and 'status' in request.GET:
                requests = Visit.objects.filter(formed_at__gte=request.GET['date'],status=request.GET['status']).exclude(formed_at=None).exclude(status='draft')
            else:
                requests = Visit.objects.exclude(status='draft')
        else:
            if 'date' in request.GET and 'status' in request.GET:
                requests = Visit.objects.filter(formed_at__gte=request.GET['date'],status=request.GET['status'],user=request.user)
            else:
                requests = Visit.objects.filter(user=request.user)
...
class GetVisits(APIView):
    ...
    def get(self, request, pk):
        ...
        events_serializer = EventsInVisitSerializer(events, many=True)
